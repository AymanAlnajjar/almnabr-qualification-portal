...
import { renderPdfHtml } from "./pdf-template.mjs";
...
export async function generatePdfForSubmission({ supabase, submissionId }) {
  ...
  const { data: updated, error: updateError } = await supabase
    .from("submissions")
    .update({ pdf_state: "ready", pdf_path: pdfPath, pdf_error: null })
    .eq("id", submissionId)
    .select("id, reference, pdf_state, pdf_path")
    .single();
